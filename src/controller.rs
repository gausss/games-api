...
#[get("/games/{game_id}")]
pub async fn get_game_by_id(path: Path<String>, game_store: Data<Mutex<InMemoryStore<Game>>>) -> HttpResponse {
...
#[put("/games")]
pub async fn update_game_by_id(game: Json<Game>, game_store: Data<Mutex<InMemoryStore<Game>>>) -> HttpResponse {
...
pub async fn delete_game_by_id(path: Path<String>, game_store: Data<Mutex<InMemoryStore<Game>>>) -> HttpResponse {
    let id = path.into_inner().parse().unwrap();
    let mut store = game_store.lock().unwrap();
...
mod tests {
    ...
    async fn test_delete_game_by_id() {
        ...
        let app = test::init_service(App::new()
            .app_data(test_store.clone())
            .service(delete_game_by_id)).await;
    ...
    async fn test_add_game() {
        ...
        let app = test::init_service(App::new()
            .app_data(test_store.clone())
            .service(get_games).service(update_game_by_id)).await;

        let req = test::TestRequest::get().uri("/games").to_request();
        let resp: Vec<Game> = test::call_and_read_body_json(&app, req).await;
        assert_eq!(resp.len(), 1);

        let req = test::TestRequest::put().uri("/games").set_json(Game::new(2, "Age of Empires")).to_request();
        let resp = test::call_service(&app, req).await;
        assert_eq!(resp.status(), StatusCode::CREATED);

        let req = test::TestRequest::put().uri("/games").set_json(Game::new(1, "Dark Souls")).to_request();
        let resp = test::call_service(&app, req).await;
